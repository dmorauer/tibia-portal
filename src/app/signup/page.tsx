'use client'
  ...
  export default function SignupPage() {
    ...
    async function handleSignup(e: React.FormEvent) {
      ...
      setTimeout(() => router.push('/character'), 1500)
    }
      ...
      return (
        <main className="min-h-screen flex items-center justify-center bg-[#1a0a05] px-4 py-12 relative overflow-hidden">
          ...
          <div className="relative w-full max-w-md bg-[#f4e4bc] rounded-md shadow-2xl border-2 border-[#8b6b3a] p-10 text-center">
            ...
            <h2 className="font-serif text-2xl text-[#3a2410] mb-2">Jornada Iniciada!</h2>
            <p className="font-serif text-[#5a3a1a]">
              Verifique seu email para confirmar (se a confirmação estiver ativada) ou aguarde ser transportado de volta à taverna.
            </p>
            <div className="mt-6 flex items-center justify-center gap-2 text-[#8b6b3a]">
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-[#1a0a05] px-4 py-12 relative overflow-hidden">
        {/* Textura de fundo + brasas flutuantes */}
        <BackgroundFlames />
        ...
        <div className="relative w-full max-w-md bg-[#f4e4bc] rounded-md shadow-2xl border-2 border-[#8b6b3a] p-10">
          ...
          <form onSubmit={handleSignup} className="space-y-5" noValidate>
            <div>
              <label htmlFor="username" className="block font-serif text-sm text-[#3a2410] mb-1.5 tracking-wide">
                Nome do Aventureiro
              </label>
              <input
                id="username"
                ...
                className="w-full rounded-sm border-2 border-[#8b6b3a] bg-[#faf0d7] px-3.5 py-2.5 font-serif text-[#3a2410] placeholder-[#a88a5a] italic transition focus:border-[#5a3a1a] focus:bg-[#fff8e1] focus:ring-2
  focus:ring-[#8b6b3a]/30 focus:outline-none"
              />
            </div>
  ...
  focus:ring-[#8b6b3a]/30 focus:outline-none"
              />
              <p className="font-serif text-xs text-[#8b6b3a] mt-1.5 italic">Mínimo de 6 caracteres</p>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-sm bg-gradient-to-b from-[#8b6b3a] to-[#5a3a1a] text-[#f4e4bc] py-3 font-serif text-lg tracking-widest uppercase border-2 border-[#3a2410] shadow-md transition hover:from-[#9a7a4a]
  hover:to-[#6a4a2a] active:translate-y-px focus:outline-none focus:ring-2 focus:ring-[#8b6b3a] focus:ring-offset-2 focus:ring-offset-[#f4e4bc] disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading ? (
          ...
          <div className="mt-6 pt-6 border-t border-[#8b6b3a]/40 text-center">
            <p className="font-serif text-sm text-[#5a3a1a]">
              ...
              <a href="/login" className="font-serif text-[#3a2410] font-semibold underline decoration-[#8b6b3a] underline-offset-4 hover:decoration-[#3a2410]">
                Voltar à taverna
              </a>
            </p>
          </div>
  ...
  // Brasas/flames flutuando no fundo (combina com "idle" + fogueira da taverna)
  function BackgroundFlames() {
    return (
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {/* Brilho dourado no centro (fogueira) */}
        <div
          className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full"
          style={{
            background: 'radial-gradient(circle, rgba(240, 200, 74, 0.12) 0%, transparent 60%)',
          }}
        />
        {/* Brasas subindo */}
  ...
  // Ornamentos nos cantos do pergaminho
